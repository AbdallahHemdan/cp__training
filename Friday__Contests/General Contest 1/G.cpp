...
const ll maxn = 5e3 + 53;
vll adj[maxn];
ll mark[maxn];

void dfs(ll u, ll color) { // mark all reacable nodes from u as (u-color)
	mark[u] = color;
	for (auto i : adj[u]) {
		if (mark[i] != color) dfs(i, color);
	}
}
void getAC() {
	ll n, m, src; sc3(n, m, src);
	for (ll i = 0; i < m; i++) {
		ll u, v; sc2(u, v);
		adj[u].push_back(v);
	}
	...
	set<ll> st;
	for (ll i = 1; i <= n; i++) {
		st.insert(mark[i]); // count number of colors in the graph..
	}
	print(st.sz - 1);
}
int main() {
