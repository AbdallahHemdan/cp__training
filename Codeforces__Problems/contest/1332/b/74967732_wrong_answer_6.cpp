...
#define SetPre(Ans, ret) fixed << setprecision(ret) << (Ans)
#define clr clear()
#define re return
...
#define isOn(S, jj) (S & (1 << jj))
#define setBit(S, jj) (S |= (1 << jj))
#define clearBit(S, jj) (S &= ~(1 << jj))
#define toggleBit(S, jj) (S ^= (1 << jj))
#define lowBit(S) (S & (-S))
...
vi getFactorial[maxm];
ll n, a[maxm];
bool okCheck = 0;

void go(ll ii) {
	for (ll jj = 3; jj <= sqrt(a[ii]); jj += 2) {
		if (a[ii] % jj == 0) {
			getFactorial[jj].push_back(ii);
			continue;
		}
	}
}
void read() {
	sc(n);
	lop(ii, 1, n + 1) {
		sc(a[ii]); (a[ii] % 2 == 0) ? getFactorial[2].push_back(ii) : go(ii);
	}
}
void clearFact(ll ii) {
	while (!getFactorial[p[ii]].empty()) {
		okCheck = 1;
		getFactorial[p[ii]].pop_back();
	}
}
void getRet() {
	ll toRet[maxm], ret = 0, cnt = 0;
	lop(ii, 0, 11) {
		if (!getFactorial[p[ii]].sz) cnt++;
		lop(jj, 0, getFactorial[p[ii]].sz) {
			ll ax = getFactorial[p[ii]][jj];
			toRet[ax] = ii + 1,
			toRet[ax] -= cnt,
			ret = ii + 1 - cnt;
		}
