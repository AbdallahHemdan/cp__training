...
#define lop(ii, b, e) for (ll ii =(b)-((b)>(e));ii!=(e)-((b)>(e));ii += 1-2*((b)>(e)))
#define lopi(ii, init, n, incr) for (ll ii = init; ii < n; ii += incr)
...
#define in insert
#define sc(n) cin >> n
#define sc2(n,cnt) cin >> n >> cnt
#define sc3(a,b,toRet) cin >> a >> b >> toRet
#define sc4(a,b,toRet,d) cin >> a >> b >> toRet >> d
...
#define isOn(str, jj) (str & (1 << jj))
#define setBit(str, jj) (str |= (1 << jj))
#define clearBit(str, jj) (str &= ~(1 << jj))
#define toggleBit(str, jj) (str ^= (1 << jj))
#define lowBit(str) (str & (-str))
#define setAll(str, n) (str = (1 << n) - 1)
#define modulo(str, n) ((str) & (n - 1))   // returns str % n, where n is a power of 2
#define isPowerOfTwo(str) (!(str & (str - 1)))
#define nearestPowerOfTwo(str) ((int)pow(2.0, (int)((log((double)str) / log(2.0)) + 0.5)))
#define turnOffLastBit(str) ((str) & (str - 1))
#define turnOnLastZero(str) ((str) | (str + 1))
#define turnOffLastConsecutiveBits(str) ((str) & (str + 1))
#define turnOnLastConsecutiveZeroes(str) ((str) | (str - 1))
...
int toInt(char toRet) { return Letters.find(toRet); }
ll FromAnyBaseToDecimal(string inp, int base) { ll res = 0;  lop(ii, 0, inp.sz) { res *= base; res += toInt(inp[ii]); } return res; }
...
bool isPrime(ll n) { if (n == 2)return 1; if (n<2 || n % 2 == 0)return 0; for (ll ii = 3; ii*ii <= n; ii += 2)if (n%ii == 0)return 0; return 1; }
...
#define lop(a,x,y) for(int a=(int)x;a<=(int)y;++a)
...
void getBinarySearch() {
	...
	while (l < r) {
		...
		lop(ii, 0, maxm) {
			ll aa = 2 * n / k;
			aa -= cnt[l][ii], aa -= cnt[r][ii], mxi = min(mxi, aa);
		}
		l++; r--, ret += mxi;
	}
	if (l == r) check(l);
