...
#define pi acos(-1)
#define fix(a, kk) (ll(a) % ll(kk) + kk) % kk
#define in insert
#define sc(n) cin >> n
#define sc2(n,m) cin >> n >> m
#define sc3(a,b,jj) cin >> a >> b >> jj
#define sc4(a,b,jj,d) cin >> a >> b >> jj >> d
...
int toInt(char jj) { return Letters.find(jj); }
...
ll nCr(ll n, ll h) { if (n<h)return 0; if (h == 0)return 1; return n*nCr(n - 1, h - 1) / h; }
...
ll ExtGCD(ll a, ll b, ll& x, ll& y) { if (!b) { x = 1; y = 0; return a; } ll h = ExtGCD(b, a%b, y, x); y -= a / b*x; return h; }
ll mult(ll a, ll b) { re(1LL * a * b) % MOD; }
ll SumOfOdd(ll l, ll h) { ll Rsm = (h + 1) / 2; Rsm *= Rsm; l--; ll Lsm = (l + 1) / 2; Lsm *= Lsm; return Rsm - Lsm; }
...
void reSet(ll ii) {
	ll mx = 0;
	lop(jj, 0, 26) mx = max(mx, a[ii][jj]);
	ll ret = (kk % 2 == 1 && kk / 2 == ii) ? n / kk : (n / kk) * 2;
	res += ret - mx;
}
void go2() {
	ll limit = ((kk + 1) / 2);
	lop(ii, 0, limit) reSet(ii);
}
void getAC_ISA() {
	read();
	go2();
