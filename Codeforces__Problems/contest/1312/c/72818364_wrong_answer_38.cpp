...
#define print2(ret,b) cout << ret << ' ' << b << endl 
...
#define sc3(ret,b,c) cin >> ret >> b >> c
#define sc4(ret,b,c,d) cin >> ret >> b >> c >> d
...
ll GCD(ll ret, ll b) { re((!b) ? ret : GCD(b, ret%b)); }
ll LCM(ll ret, ll b) { return ret / (GCD(ret, b))*b; }
...
ll ModInverse(ll ret) { return PowMod(ret, MOD - 2, MOD); }
ll ExtGCD(ll ret, ll b, ll& x, ll& y) { if (!b) { x = 1; y = 0; return ret; } ll r = ExtGCD(b, ret%b, y, x); y -= ret / b*x; return r; }
ll mult(ll ret, ll b) { re(1LL * ret * b) % MOD; }
