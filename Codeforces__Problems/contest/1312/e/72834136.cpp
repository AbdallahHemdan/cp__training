...
const ll maxn = 1111111;
const ll maxm = 1e3 + 13;
ll n;
ll a[maxm][maxm];
ll dp[maxm][maxm];
void read() {
	sc(n);
	lop(i, 1, n + 1) sc(a[i][i]);
}
void goProcess(ll i, ll ii) {
	lop(ans, i, i + ii) {
		ll aa = dp[i][i + ii - 1];
		ll bb = dp[ans + 1][i + ii - 1];
		ll cc = dp[i][ans] + bb;
		dp[i][i + ii - 1] = max(aa, cc);
		if (a[i][ans]) {
			if (a[ans + 1][i + ii - 1]) {
				ll jj = a[i][ans];
				ll kk = a[ans + 1][i + ii - 1];
				if (jj == kk) {
					a[i][i + ii - 1] = a[i][ans];
					a[i][i + ii - 1]++;
				}
...
ll processAllElements(ll i, ll ii) {
	if (i + ii - 1 > n) return 0;
	else return goProcess(i ,ii), 1;
}
void getAns(ll ii) {
	lop(i,1,n+1) {
		ll ok = processAllElements(i, ii);
		if (!ok) break;
	}
}
void getACISA() {
	read();
	lop(ii, 1, n + 1) getAns(ii);
	ll ret = n; 
	ret -= dp[1][n];
	print(ret);
}
