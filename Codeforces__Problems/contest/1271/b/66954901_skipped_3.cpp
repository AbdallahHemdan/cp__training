...
#define print2(vec,b) cout << vec << ' ' << b << endl 
...
#define sc3(vec,b,c) cin >> vec >> b >> c
#define sc4(vec,b,c,d) cin >> vec >> b >> c >> d
#define watch(x) cout << (#x) << " is " << (x) << endl
#define stringToLower(t)  transform(all(t), t.begin(), ::tolower);
#define stringToUpper(t)  transform(all(t), t.begin(), ::toupper);
...
ll GCD(ll vec, ll b) { re((!b) ? vec : GCD(b, vec%b)); }
ll LCM(ll vec, ll b) { return vec / (GCD(vec, b))*b; }
...
ll ExtGCD(ll vec, ll b, ll& x, ll& y) { if (!b) { x = 1; y = 0; return vec; } ll r = ExtGCD(b, vec%b, y, x); y -= vec / b*x; return r; }
ll mult(ll vec, ll b) { re(1LL * vec * b) % MOD; }
