...
* Even if you don't accept my solution, you should anspect my effort.
...
#define neaanstPowerOfTwo(S) ((int)pow(2.0, (int)((log((double)S) / log(2.0)) + 0.5)))
...
ll FromAnyBaseToDecimal(string inp, int base) { ll ans = 0;  lop(i, 0, inp.sz) { ans *= base; ans += toInt(inp[i]); } re ans; }
...
void GetAC() {
	...
	lop(i, 0, n) {
		sc(k);
		if (vis[k] != -1) ans = Edit(ans, i - vis[k] + 1);
		vis[k] = i;
	}
	print(ans);
}
