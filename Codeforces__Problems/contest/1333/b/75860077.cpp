...
#define pi acos(-1)
#define fix(g, k) (ll(g) % ll(k) + k) % k
#define in insert
#define sc(n) cin >> n
#define sc2(n,m) cin >> n >> m
#define sc3(g,bi,c) cin >> g >> bi >> c
#define sc4(g,bi,c,d) cin >> g >> bi >> c >> d
...
void go(ll j) {
	...
	if (ai[j] < bi[j] && !aa) ok = 1;
	else if (ai[j] > bi[j] && !bb) ok = 1;
}
void getAC_ISA() {
	read();
	for (ll j = n - 1; j >= 0; j--) go(j);
	(ok) ? print("NO") : print("YES");
}
