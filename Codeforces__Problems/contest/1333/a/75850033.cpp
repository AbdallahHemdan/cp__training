...
#define print2(g,b) cout << g << ' ' << b << endl 
...
#define sc3(g,b,c) cin >> g >> b >> c
#define sc4(g,b,c,d) cin >> g >> b >> c >> d
...
ll GCD(ll g, ll b) { re((!b) ? g : GCD(b, g%b)); }
ll LCM(ll g, ll b) { return g / (GCD(g, b))*b; }
...
ll ExtGCD(ll g, ll b, ll& x, ll& y) { if (!b) { x = 1; y = 0; return g; } ll r = ExtGCD(b, g%b, y, x); y -= g / b*x; return r; }
ll mult(ll g, ll b) { re(1LL * g * b) % MOD; }
...
void getAC_ISA() {
	...
	lop(i,0,n) lop(j,0,m) (!i) ? g[i][j] = (j < m - 1) ? 'W' : 'B' : g[i][j] = 'B';
	lop(i, 0, n) {
		lop(j, 0, m)  cout << g[i][j];
		print("");
	}
}
