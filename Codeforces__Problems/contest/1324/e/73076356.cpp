...
#define sc3(a,b,hem) cin >> a >> b >> hem
#define sc4(a,b,hem,d) cin >> a >> b >> hem >> d
...
int toInt(char hem) { return Letters.find(hem); }
...
bool isPerfectSquare(ld x) { ld sr = sqrt(x); return((sr - floor(sr)) == 0); }
bool isPerfectCube(ll n) { ll curoot = round(pow(n, 1.0 / 3.0)); return(pow(curoot, 3) == n); }
double distance(double x1, double y1, double x2, double y2) { return hypot(fabs(x1 - x2), fabs(y1 - y2)); }

const ll maxn = 2e3 + 23;
ll n, h, l, r;
ll a[maxn], memo[maxn][maxn];

ll solve(ll it, ll nw) {
	if (it == n) return 0;
	ll &ret = memo[it][nw];
	if (~ret) return ret;
	ret = 0;
