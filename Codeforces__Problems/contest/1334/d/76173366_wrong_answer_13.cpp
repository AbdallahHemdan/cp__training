...
void go3() {
	if ((l - ret) % 2 == 0) {
		out.PB(it + (l - ret) / 2);
		if (it + (l - ret) / 2 != n) { lt = it; nt = it + (l - ret) / 2 + 1; }
		else { lt = it + 1; nt = it + 2; }
	}
	else {
		ll aa = it + (l - ret) / 2 + 1;
		lt = it;
	...
	while (out.size() < r - l + 1) {
		out.push_back(lt), out.push_back(nt);
		if (nt == n) {
			(lt == n - 1) ? lt = 1 :
				lt++, nt = lt + 1;
		}
