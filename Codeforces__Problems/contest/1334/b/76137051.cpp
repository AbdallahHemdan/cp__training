...
#define lop(j, bi, e) for (ll j = bi; j < e ; j++)
...
#define print2(g,bi) cout << g << ' ' << bi << endl 
#define vpii vector<pair<int, int> >
#define vpll vector<pair<ll, ll> >
#define pll pair<ll, ll>
#define pii pair<int,int>
#define pi acos(-1)
#define fix(g, ko) (ll(g) % ll(ko) + ko) % ko
#define in insert
#define sc(n) cin >> n
#define sc2(n,m) cin >> n >> m
#define sc3(g,bi,ret) cin >> g >> bi >> ret
#define sc4(g,bi,ret,a) cin >> g >> bi >> ret >> a
...
#define modulo(S, N) ((S) & (N - 1))   // returns S % N, where N is g power of 2
...
#else
inline int __builtin_popcountll(__int64 g) {
	return __builtin_popcount((ui32)g) + __builtin_popcount(g >> 32);
}
#endif
#endif
...
int toInt(char ret) { return Letters.find(ret); }
ll FromAnyBaseToDecimal(string inp, int base) { ll res = 0;  lop(j, 0, inp.sz) { res *= base; res += toInt(inp[j]); } return res; }
string FromDecimalToAnyBase(ll n, int base) { if (n == 0) return "0";  string ret = ""; while (n) { ret = Letters[n%base] + ret;  n /= base; } return ret; }
ll GCD(ll g, ll bi) { re((!bi) ? g : GCD(bi, g%bi)); }
ll LCM(ll g, ll bi) { return g / (GCD(g, bi))*bi; }
ll nCr(ll n, ll r) { if (n<r)return 0; if (r == 0)return 1; return n*nCr(n - 1, r - 1) / r; }
ll PowMod(ll bs, ll ex, ll M) { if (!ex)return 1; ll ai = PowMod((bs*bs) % M, ex >> 1, M); re(ex & 1) ? (ai*bs) % M : ai; }
ll fastpow(ll bi, ll e) { if (!e) return 1; if (e & 1) return bi * fastpow(bi, e - 1); ll x = fastpow(bi, e / 2); return x * x; }
ll ModInverse(ll g) { return PowMod(g, MOD - 2, MOD); }
ll ExtGCD(ll g, ll bi, ll& x, ll& y) { if (!bi) { x = 1; y = 0; return g; } ll r = ExtGCD(bi, g%bi, y, x); y -= g / bi*x; return r; }
ll mult(ll g, ll bi) { re(1LL * g * bi) % MOD; }
...
bool isPrime(ll n) { if (n == 2)return 1; if (n<2 || n % 2 == 0)return 0; for (ll j = 3; j*j <= n; j += 2)if (n%j == 0)return 0; return 1; }
...
void getAC_ISA() {
	read();
	if (ag >= x) return print(n), void();
	go();
	lop(i, 0, n) if (go2(i) == -1) break;
	print(ret);
}
