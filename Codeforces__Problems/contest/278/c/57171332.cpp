...
ll NoOfConnectedComponents(ll n) {
	ll cnt = 0;
	lop(i, 1, n + 1) if (findParent(i) == i) cnt++;
	re cnt;
}
...
int main() {
	...
	if (ok == n) re print(n), 0; 
	for (auto i : mp) {
		if (i.yy.sz == 1) continue;
		lop(j, 0, i.yy.sz) {
			lop(k, j + 1, i.yy.sz) {
				Merge(i.yy[j], i.yy[k]);
			}
	...
	print(NoOfConnectedComponents(n) - 1);
}