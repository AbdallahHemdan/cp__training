...
#define watch(t) cout << (#t) << " is " << (t) << endl
...
ll fastpow(ll b, ll e) { if (!e) return 1; if (e & 1) return b * fastpow(b, e - 1); ll t = fastpow(b, e / 2); return t * t; }
ll ModInverse(ll a) { return PowMod(a, MOD - 2, MOD); }
ll ExtGCD(ll a, ll b, ll& t, ll& s) { if (!b) { t = 1; s = 0; return a; } ll r = ExtGCD(b, a%b, s, t); s -= a / b*t; return r; }
...
bool isPerfectSquare(ld t) { ld sr = sqrt(t); re((sr - floor(sr)) == 0); }
...
void print1() {
	ll aa = xx + yy;
	cout << 2 << endl << aa << ' ' << yy << endl;
}
void print22() {
	cout << 3 << endl << xx << ' ' << yy << ' ' << zz << endl;
}
void go2() {
	if ((xx + yy) == (xx^yy)) print1();
	else print22();
}
void go() {
	xx = a;
