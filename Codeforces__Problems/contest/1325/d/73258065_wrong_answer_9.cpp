...
#define Ceil(aaa, bbb) ((aaa + bbb - 1) / bbb)
...
#define watch(aaa) cout << (#aaa) << " is " << (aaa) << endl
...
ll fastpow(ll b, ll e) { if (!e) return 1; if (e & 1) return b * fastpow(b, e - 1); ll aaa = fastpow(b, e / 2); return aaa * aaa; }
ll ModInverse(ll a) { return PowMod(a, MOD - 2, MOD); }
ll ExtGCD(ll a, ll b, ll& aaa, ll& bbb) { if (!b) { aaa = 1; bbb = 0; return a; } ll r = ExtGCD(b, a%b, bbb, aaa); bbb -= a / b*aaa; return r; }
...
bool PointInRectangle(ll x1, ll y1, ll x2,
	ll y2, ll aaa, ll bbb) {
	return (aaa >= x1 && aaa <= x2 && bbb >= y1 && bbb <= y2);
}
...
bool isPerfectSquare(ld aaa) { ld sr = sqrt(aaa); re((sr - floor(sr)) == 0); }
...
void go() {
	...
	ll bbb = (b - a) / 2; 
	ll ccc = (b - a) / 2;
	bool ok = (aaa + bbb) == (aaa^bbb);
	ll aa = aaa + bbb;
	if (ok) print(2), cout << aa << ' ' << bbb << endl;
	else print(3), cout << aaa << ' ' << bbb << ' ' << ccc << endl;
}
void goCheck() {
	bool ok = (a > b || a % 2 != b % 2);
	if (ok) print(-1);
	else if (!a && !b) print(0);
	else if (a == b) print(1), print(a);
	else go();
}
void getACISA() {
	sc2(a, b);
