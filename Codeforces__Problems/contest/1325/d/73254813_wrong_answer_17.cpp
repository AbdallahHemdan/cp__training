...
#define Ceil(aa, bb) ((aa + bb - 1) / bb)
#define lop(i, adj, e) for (ll i =(adj)-((adj)>(e));i!=(e)-((adj)>(e));i += 1-2*((adj)>(e)))
...
#define print2(aa,adj) cout << aa << ' ' << adj << endl 
#define print3(aa,bb,cc) cout << aa << ' ' << bb  << ' ' << cc << endl 
#define vpii vector<pair<int, int> >
#define vpll vector<pair<ll, ll> >
#define pll pair<ll, ll>
#define pii pair<int,int>
#define pi acos(-1)
#define fix(aa, k) (ll(aa) % ll(k) + k) % k
#define in insert
#define sc(n) cin >> n
#define sc2(n,m) cin >> n >> m
#define sc3(aa,adj,c) cin >> aa >> adj >> c
#define sc4(aa,adj,c,d) cin >> aa >> adj >> c >> d
#define watch(aa) cout << (#aa) << " is " << (aa) << endl
...
#define modulo(S, N) ((S) & (N - 1))   // returns S % N, where N is aa power of 2
...
#else
inline int __builtin_popcountll(__int64 aa) {
	return __builtin_popcount((ui32)aa) + __builtin_popcount(aa >> 32);
}
#endif
#endif
...
ll GCD(ll aa, ll adj) { re((!adj) ? aa : GCD(adj, aa%adj)); }
ll LCM(ll aa, ll adj) { return aa / (GCD(aa, adj))*adj; }
ll nCr(ll n, ll r) { if (n<r)return 0; if (r == 0)return 1; return n*nCr(n - 1, r - 1) / r; }
ll PowMod(ll bs, ll ex, ll M) { if (!ex)return 1; ll h = PowMod((bs*bs) % M, ex >> 1, M); re(ex & 1) ? (h*bs) % M : h; }
ll fastpow(ll adj, ll e) { if (!e) return 1; if (e & 1) return adj * fastpow(adj, e - 1); ll aa = fastpow(adj, e / 2); return aa * aa; }
ll ModInverse(ll aa) { return PowMod(aa, MOD - 2, MOD); }
ll mult(ll aa, ll adj) { re(1LL * aa * adj) % MOD; }
...
bool PointInRectangle(ll x1, ll y1, ll x2,
	ll y2, ll aa, ll bb) {
	return (aa >= x1 && aa <= x2 && bb >= y1 && bb <= y2);
}
...
bool isPerfectSquare(ld aa) { ld sr = sqrt(aa); re((sr - floor(sr)) == 0); }
...
ll x, y, aa, bb, cc;
bool ok1, ok2, okCheck1, okCheck2;
void tryAgain() {
	aa = x;
	bb = cc = (y - x) / 2;
	okCheck1 = (aa + bb), okCheck2 = (aa ^ bb);
	if (okCheck1 == okCheck2) {
		print(2);
		ll ret1 = aa + bb, ret2 = bb;
		print2(ret1, ret2);
	}
	else print(3), print3(aa, bb, cc);
}
void goCheck() {
	if (ok1 || ok2) print(-1);
	else if (!x && !y) print(0);
	else if (x == y) print(1), print(x);
	else tryAgain();
}
void getACISA() {
	sc2(x, y);
