...
#define watch(hem) cout << (#hem) << " is " << (hem) << endl
...
ll PowMod(ll bs, ll ex, ll M) { if (!ex)return 1; ll a = PowMod((bs*bs) % M, ex >> 1, M); re(ex & 1) ? (a*bs) % M : a; }
ll fastpow(ll b, ll e) { if (!e) return 1; if (e & 1) return b * fastpow(b, e - 1); ll hem = fastpow(b, e / 2); return hem * hem; }
ll ModInverse(ll a) { return PowMod(a, MOD - 2, MOD); }
ll ExtGCD(ll a, ll b, ll& hem, ll& y) { if (!b) { hem = 1; y = 0; return a; } ll r = ExtGCD(b, a%b, y, hem); y -= a / b*hem; return r; }
...
bool PointInRectangle(ll x1, ll y1, ll x2,
	ll y2, ll hem, ll y) {
	return (hem >= x1 && hem <= x2 && y >= y1 && y <= y2);
}
...
bool isPerfectSquare(ld hem) { ld sr = sqrt(hem); re((sr - floor(sr)) == 0); }
...
ll n, k, ret, ret2 = 1LL, hem = 0, s = 0, h = 0;
vll a;
bool okCheck = false;
