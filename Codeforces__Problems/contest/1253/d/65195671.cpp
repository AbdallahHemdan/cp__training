...
#define turnOfflstBit(S) ((S) & (S - 1))
#define turnOnlstZero(S) ((S) | (S + 1))
#define turnOfflstConsecutiveBits(S) ((S) & (S + 1))
#define turnOnlstConsecutiveZeroes(S) ((S) | (S - 1))
...
int mxi[MAXN], par[MAXN], sz[MAXN];
int n, m;
int findParent(int v) {
	re v == par[v] ? v : par[v] = findParent(par[v]);
}
void Merge(int v, int u) {
	...
	par[u] = v;
	sz[v] += sz[u];
	mxi[v] = max(mxi[v], mxi[u]);
}
void Process() {
	int lst = par[findParent(1)];
	int mxi1 = mxi[findParent(1)];
	int ans = 0;
	lop(i, 1, n + 1) {
		int k = findParent(i);
		if (mxi1 > i && k != lst) {
