...
* Even if you don't accept my solution, you should respect my exxort.
...
int main() {
	...
	map<char, int> mapofs; 
	map<char, int> mapoft; 
	for (auto c : s) mapofs[c] ++;
	for (auto c : t) mapoft[c] ++;
	ll yay = 0, opps = 0; 
	for (auto i : mapofs) {
		if (mapoft[i.xx]) {
			int tmp = min(mapoft[i.xx], i.yy);
			yay += tmp; 
			mapofs[i.xx] -= tmp;
			mapoft[i.xx] -= tmp;
		}
	...
	for (auto it : mapofs) {
		if (isupper(it.xx)) {
			int tmp = min(mapofs[it.xx], mapoft[it.xx + 32]);
			opps += tmp; mapofs[it.xx] -= tmp; mapoft[it.xx + 32] -= tmp;
		}
		else {
			int tmp = min(mapofs[it.xx], mapoft[it.xx - 32]);
			opps += tmp; mapofs[it.xx] -= tmp; mapoft[it.xx - 32] -= tmp;
