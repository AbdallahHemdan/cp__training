...
ll c[maxn];
ll okCheck = 1;
ll process(ll ok, ll x) { // ok = 1 => n, ok = 2 => m
	bck = 0, mms(c, 0);
	ll lmt = (ok == 1) ? n : m; 
	lop(i, 1, lmt + 1) {
		c[i] = c[i - 1];
		(ok == 1) ? c[i] += a[i] : c[i] += b[i];
	}
	ll mxi = lmt - x + 2;
	lop(i, 1, mxi) {
		ll val = c[x + i - 1] - c[i - 1];
		if (val == x) bck++;
		else okCheck = 1;
	}
	return bck;
...
void check(ll i) {
	if (k%i);
		...
		ll toAdd = process(1, k / i) * process(2, i);
		ans += toAdd;
		if (i != k / i) {
			ll toAdd = process(1, i)*process(2, k / i);
			ans += toAdd;
		}
	}
}
