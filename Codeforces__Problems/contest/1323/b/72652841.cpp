...
void okCheck() {
	if (!a) {
		(toCheck) ? sol2[toCheck] ++, toCheck = 0 : toCheck = 0;
	}
	else toCheck++;
}
void reProcess() {
...
void getAns() {
	for (auto ii : sol1) for (auto jj : sol2) toAdd = call(jj.first, ii.first) * ii.second* jj.second, ans += toAdd;
}
void getAC() {
	process();
	reProcess();
