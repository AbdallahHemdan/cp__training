...
#define sc3(a,b,rt) cin >> a >> b >> rt
#define sc4(a,b,rt,d) cin >> a >> b >> rt >> d
...
int toInt(char rt) { return Letters.find(rt); }
...
vll adj[maxn], lst;
ll in[maxn], lvl[maxn], rt = 1, hem[maxn], frq, od = 0, evn = 0, mn = 1, mx, n, st = -1;
void Go1(ll u, ll h) { for (ll v : adj[u]) if (v != h) { lvl[v] = lvl[u] + 1; Go1(v, u); } }
void Go2(ll u, ll h) {
	ll cc = 0;
	for (ll v : adj[u]) if (v != h) if (hem[v]) cc++;
	if (cc > 0) frq += rt, frq--;
	for (ll v : adj[u]) if (v != h) Go2(v, u);
}
void read() {
	sc(n);
	lop(i, 0, n - 1) {
...
void go1() {
	lop(i, 1, n + 1) if (in[i]>1) { rt = i; break; }
	lvl[rt] = 1;
}
void go2() {
	Go1(rt, -1);
	lop(i, 1, n + 1) if (in[i] == 1) lst.PB(i), hem[i] = 1;
}
void go3() {
	for (ll u : lst) (lvl[u] % 2) ? od++ : evn++;
	if (od > 0) if (evn > 0) mn = 3;
	mx = n - 1,
	frq = 0;
}
void get() {
