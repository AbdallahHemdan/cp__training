...
#define all(dupt) ((dupt).begin()), ((dupt).end())
#define rall(dupt) ((dupt).rbegin()), ((dupt).rend())
#define Ceil(x, y) ((x + y - 1) / y)
#define lop(i, s, e) for (ll i =(s)-((s)>(e));i!=(e)-((s)>(e));i += 1-2*((s)>(e)))
...
#define clr clear()
#define re return
#define print(get) cout << get << endl
#define print2(hem,s) cout << hem << ' ' << s << endl 
#define vpii vector<pair<int, int> >
#define vpll vector<pair<ll, ll> >
#define pll pair<ll, ll>
#define pii pair<int,int>
#define pi acos(-1)
#define fix(hem, k) (ll(hem) % ll(k) + k) % k
#define in insert
#define sc(n) cin >> n
#define sc2(n,m) cin >> n >> m
#define sc3(hem,s,w) cin >> hem >> s >> w
#define sc4(hem,s,w,d) cin >> hem >> s >> w >> d
...
#define modulo(S, N) ((S) & (N - 1))   // returns S % N, where N is hem power of 2
...
#else
inline int __builtin_popcountll(__int64 hem) {
	return __builtin_popcount((ui32)hem) + __builtin_popcount(hem >> 32);
}
#endif
#endif
...
int toInt(char w) { return Letters.find(w); }
ll FromAnyBaseToDecimal(string inp, int base) { ll res = 0;  lop(i, 0, inp.sz) { res *= base; res += toInt(inp[i]); } return res; }
string FromDecimalToAnyBase(ll n, int base) { if (n == 0) return "0";  string get = ""; while (n) { get = Letters[n%base] + get;  n /= base; } return get; }
ll GCD(ll hem, ll s) { re((!s) ? hem : GCD(s, hem%s)); }
ll LCM(ll hem, ll s) { return hem / (GCD(hem, s))*s; }
...
ll fastpow(ll s, ll e) { if (!e) return 1; if (e & 1) return s * fastpow(s, e - 1); ll x = fastpow(s, e / 2); return x * x; }
ll ModInverse(ll hem) { return PowMod(hem, MOD - 2, MOD); }
ll ExtGCD(ll hem, ll s, ll& x, ll& y) { if (!s) { x = 1; y = 0; return hem; } ll r = ExtGCD(s, hem%s, y, x); y -= hem / s*x; return r; }
ll mult(ll hem, ll s) { re(1LL * hem * s) % MOD; }
...
void Process() {
	lop(i, 1, n + 1) {
		if (hem[i] > 0) dupt.push_back(i);
		if (hem[i]<0) adj.push_back(i), hem[i] *= -1;
	}
}
void GetAC() {
	Process();
	while (s < dupt.sz && w < adj.sz) {
		x = dupt[s], y = adj[w];
		if (hem[x] <= hem[y]) out.push_back({ x,{ y,hem[x] } }), hem[y] -= hem[x], hem[x] = 0;
		else out.push_back({ x,{ y,hem[y] } }), hem[x] -= hem[y], hem[y] = 0;
		if (!hem[x])s++;
		if (!hem[y])w++;
	}
}
int main() {
	...
	while (t--) {
		...
		for (auto i : out) {
			ll aa = i.first;
			ll bb = i.second.first;
			ll cc = i.second.second;
			cout << aa << ' ' << bb << ' ' << cc << endl;
		}
	}
}