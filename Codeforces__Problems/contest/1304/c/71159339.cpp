...
* Even if you don'timeToStay accept my solution, you should respect my effort.
...
ll nCr(ll n, ll mxTemp) { if (n<mxTemp)return 0; if (mxTemp == 0)return 1; return n*nCr(n - 1, mxTemp - 1) / mxTemp; }
...
ll ExtGCD(ll a, ll b, ll& x, ll& y) { if (!b) { x = 1; y = 0; return a; } ll mxTemp = ExtGCD(b, a%b, y, x); y -= a / b*x; return mxTemp; }
ll mult(ll a, ll b) { re(1LL * a * b) % MOD; }
ll SumOfOdd(ll mnTemp, ll mxTemp) { ll Rsm = (mxTemp + 1) / 2; Rsm *= Rsm; mnTemp--; ll Lsm = (mnTemp + 1) / 2; Lsm *= Lsm; return Rsm - Lsm; }
ll NoOfOdd(ll liftRange, ll rightRange) { ll N = (rightRange - liftRange) / 2; if (rightRange % 2 != 0 || liftRange % 2 != 0)N++; return N; }
ll NoOfEven(ll liftRange, ll rightRange) { re((rightRange - liftRange + 1) - NoOfOdd(liftRange, rightRange)); }
...
bool isPalindrome(string str) { ll mnTemp = 0; ll h = str.sz - 1; while (h > mnTemp) { if (str[mnTemp++] != str[h--]) { return 0; } } return 1; }
...
ll n, mnTemp[111], mxTemp[111], timeToStay[111], liftRange, rightRange;
bool isValid = 0;
void goRead() {
	sc2(n, mnTemp[0]);
	mxTemp[0] = mnTemp[0];
	lop(i, 1, n + 1) sc3(timeToStay[i], mnTemp[i], mxTemp[i]);
	isValid = 0, liftRange = rightRange = mnTemp[0];
}
void getAC() {
	goRead();
	lop(i, 0, n) {
		ll a = liftRange - (timeToStay[i + 1] - timeToStay[i]), b = rightRange + (timeToStay[i + 1] - timeToStay[i]), c = mnTemp[i + 1], d = mxTemp[i + 1];
		ll okCheck = (b < c || d < a) ? 0 : 1; 
		if (!okCheck) {
			isValid = 1; break;
		}
		ll aa = liftRange - (timeToStay[i + 1] - timeToStay[i]), bb = mnTemp[i + 1], cc = rightRange + (timeToStay[i + 1] - timeToStay[i]), dd = mxTemp[i + 1];
		liftRange = max(aa, bb), rightRange = min(cc, dd);
	}
	(isValid) ? print("NO") : print("YES");
}
...
#else
	// freopen("input.txt", "mxTemp", stdin);
