...
vs hem;
string tmp, ret;
vector<pair<string, string>> allInOne;
void doProcessing() {
	sc2(n, m);
	lop(i, 0, n) {
		string s, toProcess; sc(s), toProcess = s;
		hem.PB(s), reverse(all(s));
		if (s == toProcess) tmp = s;
	}
	sort(all(hem));
	lop(i, 0, hem.sz) {
		string toProcess = hem[i], ask;
		reverse(all(toProcess));
		if (binary_search(hem.begin() + i + 1, hem.end(), toProcess)) allInOne.push_back({ hem[i] , toProcess });
	}
}
void getAC() {
	doProcessing();
	lop(i, 0, allInOne.sz) ret += allInOne[i].xx;
	ret += tmp;
	for (int i = allInOne.size() - 1; i >= 0; i--) ret += allInOne[i].yy;
	print(ret.sz), print(ret);
}
