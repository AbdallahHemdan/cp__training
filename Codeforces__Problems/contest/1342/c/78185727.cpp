...
ll FromAnyBaseToDecimal(string inp, int base) { ll ret = 0;  lop(i, 0, inp.sz) { ret *= base; ret += toInt(inp[i]); } return ret; }
...
ll go(ll bb) {
	ll ret = (bb / (a*b / GCD(a, b)))*((a*b / GCD(a, b)) - a);
	bb %= (a*b / GCD(a, b));
	if (bb >= a) ret += (bb - a + 1);
	return ret;
}
