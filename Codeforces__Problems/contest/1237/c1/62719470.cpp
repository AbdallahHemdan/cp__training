...
bool PointInRectangle(ll x_1, ll y_1, ll x_2,
	ll y_2, ll x, ll y) {
	re(x >= x_1 && x <= x_2 && y >= y_1 && y <= y_2);
}
...
double distanceance(double x_1, double y_1, double x_2, double y_2) { re hypot(fabs(x_1 - x_2), fabs(y_1 - y_2)); }
...
void AC_ISHA() {
	lop(i, 0, n) {
		...
		lop(j, 0, n) {
			...
			ll y_2 = ar[j].yy.xx;
			ll z_2 = ar[j].yy.yy;
			ll d = (x_1 - x_2) * (x_1 - x_2);
			d += (y_1 - y_2) * (y_1 - y_2); 
			d += (z_1 - z_2) * (z_1 - z_2);
			if (d < distance && check) distance = d, ind = j;
		}
		vis[i] = 1;
		vis[ind] = 1;
		cout << i + 1 << " " << ind + 1 << endl;
