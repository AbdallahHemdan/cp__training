...
#define clr clear()
#define re return
#define print(cnt) cout << cnt << endl
...
#define modulo(S, maxn) ((S) & (maxn - 1))   // returns S % maxn, where maxn is a power of 2
...
string FromDecimalToAnyBase(ll n, int base) { if (n == 0) return "0";  string cnt = ""; while (n) { cnt = Letters[n%base] + cnt;  n /= base; } return cnt; }
...
ll NoOfOdd(ll L, ll R) { ll maxn = (R - L) / 2; if (R % 2 != 0 || L % 2 != 0)maxn++; return maxn; }
...
string ret;
ll n, m, k, cnt;
ll x1[205], y1[205], x2[205], y2[205];


void read() {
	sc3(n, m, k);
	lop(it, 0, k) sc2(x1[it], y1[it]);
	lop(it, 0, k) sc2(x2[it], y2[it]);
}

void go() {
	cnt = 0, ret = "";
	lop(it, 0, m - 1) ret += 'L', cnt++;
	lop(it, 0, n - 1) ret += 'U', cnt++;
}
void go1(ll it) {
	lop(jj, 0, m - 1)
		ret += 'R', cnt++;
...
void go2(ll it) {
	lop(jj, 0, m - 1) ret += 'L', cnt++;
	if (it < n - 1) ret += 'D', cnt++;
}
void getRet() {
	lop(it, 0, n) (it % 2 == 0) ? go1(it) : go2(it);
	print(cnt);
	print(ret);
}
void getAC_ISA() {
