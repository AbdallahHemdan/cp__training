...
inline int __builtin_popcountll(__int64 a) {
	re __builtin_popcount((ui32)a) + __builtin_popcount(a >> 32);
}
#endif
#endif
...
class monotonic_queue {
	...
	void add(stack<pair<T, pair<T, T>>>& s, T v) {
		T mx = max(v, s.top().yy.xx);
		T mn = min(v, s.top().yy.yy);
		s.push({ v,{ mx, mn } });
	}
...
int main() {
	...
	IO;
	ll n; cin >> n; 
	monotonic_queue<ll> monoq; 
	ll ans = -OO;
	lop(i, 0, n) {
		ll x; cin >> x;
		monoq.push(x);
		while (monoq.max() - monoq.min() > 1) monoq.pop();
		ans = max(ans, monoq.sz);
	}
	print(ans);
}
