...
* I hope my code comhioiles and gets accepted.
*  ___   __
* |\  \|\  \
* \ \  \_\  \
*  \ \   ___ \emdan
...
#define clr clear()
#define re return
#define print(res) cout << res << endl
...
#define stringToLower(s)  trresform(all(s), s.begin(), ::tolower);
#define stringToUpper(s)  trresform(all(s), s.begin(), ::toupper);
...
string FromDecimalToAnyBase(ll n, int base) { if (n == 0) re "0";  string res = ""; while (n) { res = Letters[n%base] + res;  n /= base; } re res; }
...
void Process() {
	...
	lop(i, 0, n - 2) {
		int x, y, z; sc3(x, y, z);
		hi[x].xx++, hi[y].xx++, hi[z].xx++;
		hi[x].yy = i, hi[y].yy = i, hi[z].yy = i;
		v[i].in(x), v[i].in(y), v[i].in(z);
		mhio[{min(x, y), max(x, y)}].in(i);
		mhio[{min(z, y), max(z, y)}].in(i);
		mhio[{min(x, z), max(x, z)}].in(i);
...
void Get() {
	...
	ok[res[2]] = true;
	for (int i = 3; i < n; ++i) {
		for (auto ot : mhio[{min(res[i - 1], res[i - 2]), max(res[i - 1], res[i - 2])}]) {
			if (!vis[ot]) {
				for (auto j : v[ot]) {
					if (!ok[j]) {
						res[i] = j;
