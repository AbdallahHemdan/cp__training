...
int main() {
	...
	int n, m; cin >> n >> m;
	par = vi(n), rnk = vi(n, 1); 
	iota(all(par), 0); // assign increasing values to vector element starting from- init
	...
	sort(all(adj), [](pair<pair<int, int>, int> a, pair<pair<int, int>, int> b) {
		re a.yy < b.yy;
	});
	int ans = 0; 
	for (int l = 0; l < m; l++) {
		int r = l;
		while (r < m && adj[l].yy == adj[r].yy) r++;
		int ln = r - l; 
		for (int k = l; k < r; k++) {
			if (find(adj[k].xx.xx) == find(adj[k].xx.yy)) ln--; 
		}
		for (int k = l; k < r; k++) {
			ln -= merge(adj[k].xx.xx, adj[k].xx.yy);
		}
		ans += ln;
		l = r - 1;
	}
