...
int main() {
	...
	vpii b(m + 1);
	for (int i = 1; i <= m; i++) cin >> b[i].xx >> b[i].yy;
	vi ans;
	int mxi = -oo;
	for (int i = 1; i <= n; i++) {
		vi tmp(a);
		for (auto p : b) {
			...
			for (int j = p.first; j <= p.second; j++) tmp[j]--;
		}
		int mx = -oo, mn = oo; 
		for (int l = 1; l <= n; l++) mx = max(mx, tmp[l]), mn = min(mn, tmp[l]);
		/*watch(mx), watch(mn)*/;
		if (mx - mn >= mxi) {
			mxi = mx - mn;
			ans.clear();
			for (int k = 1; k <= m; k++){
				if (i >= b[k].first && i <= b[k].second) continue;
				ans.push_back(k);
			}
		}
	}
