...
#define SetPre(result, num) fixed << setprecision(num) << (result)
#define clr clear()
#define re return
...
#define stringToLower(s)  trresultform(all(s), s.begin(), ::tolower);
#define stringToUpper(s)  trresultform(all(s), s.begin(), ::toupper);
...
string FromDecimalToAnyBase(ll n, int base) { if (n == 0) re "0";  string result = ""; while (n) { result = Letters[n%base] + result;  n /= base; } re result; }
...
ll accOfOdd(ll l, ll r) { ll Rsm = (r + 1) / 2; Rsm *= Rsm; l--; ll Lsm = (l + 1) / 2; Lsm *= Lsm; re Rsm - Lsm; }
...
ll result = 0;
void Go_Get_AC() {
	lop(i,0,a.sz) if (a[i] >= 2) result += (a[i])*(a[i] - 1) / 2;
	lop(i,1,a.sz){
		if (a[i] >= 2 && a[i - 1] >= 2) {
			result += (a[i] - 1)*(a[i - 1] - 1);
		}
	}
	...
	lop(i,2,a.sz) result += a[i] * acc[i - 2];
	print(result);
}
int main() {
#ifndef ONLINE_JUDGE
