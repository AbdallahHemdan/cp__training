...
#define lop(i, arr, e) for (ll i =(arr)-((arr)>(e));i!=(e)-((arr)>(e));i += 1-2*((arr)>(e)))
...
#define print2(vec,arr) cout << vec << ' ' << arr << endl 
#define vpii vector<pair<int, int> >
#define vpll vector<pair<ll, ll> >
#define pll pair<ll, ll>
#define pii pair<int,int>
#define pi acos(-1)
#define fix(vec, k) (ll(vec) % ll(k) + k) % k
#define in insert
#define sc(n) cin >> n
#define sc2(n,m) cin >> n >> m
#define sc3(vec,arr,c) cin >> vec >> arr >> c
#define sc4(vec,arr,c,d) cin >> vec >> arr >> c >> d
...
#define modulo(S, N) ((S) & (N - 1))   // returns S % N, where N is vec power of 2
...
#else
inline int __builtin_popcountll(__int64 vec) {
	return __builtin_popcount((ui32)vec) + __builtin_popcount(vec >> 32);
}
#endif
#endif
...
ll GCD(ll vec, ll arr) { re((!arr) ? vec : GCD(arr, vec%arr)); }
ll LCM(ll vec, ll arr) { return vec / (GCD(vec, arr))*arr; }
...
ll fastpow(ll arr, ll e) { if (!e) return 1; if (e & 1) return arr * fastpow(arr, e - 1); ll x = fastpow(arr, e / 2); return x * x; }
ll ModInverse(ll vec) { return PowMod(vec, MOD - 2, MOD); }
ll ExtGCD(ll vec, ll arr, ll& x, ll& y) { if (!arr) { x = 1; y = 0; return vec; } ll r = ExtGCD(arr, vec%arr, y, x); y -= vec / arr*x; return r; }
ll mult(ll vec, ll arr) { re(1LL * vec * arr) % MOD; }
ll SumOfOdd(ll acum, ll r) { ll Rsm = (r + 1) / 2; Rsm *= Rsm; acum--; ll Lsm = (acum + 1) / 2; Lsm *= Lsm; return Rsm - Lsm; }
...
bool isPalindrome(string str) { ll acum = 0; ll h = str.sz - 1; while (h > acum) { if (str[acum++] != str[h--]) { return 0; } } return 1; }
