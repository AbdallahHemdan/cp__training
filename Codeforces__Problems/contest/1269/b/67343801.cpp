...
ll FromAnyBaseToDecimal(string inp, int base) { ll ans = 0;  lop(it, 0, inp.sz) { ans *= base; ans += toInt(inp[it]); } return ans; }
...
ll vec[maxn], arr[maxn]; 
unordered_map<int, int> frq, check;
ll n, Md;
ll mn = OO;
bool ok = true;
void Read() {
	sc2(n, Md);
	lop(i, 0, n) sc(vec[i]);
	lop(i, 0, n) sc(arr[i]), frq[arr[i]]++;
}
void GetAC() {
	Read();
	lop(i, 0, n) {
		ll toComp = ((arr[i] - vec[0] + Md) % Md);
		lop(j, 0, n) check[(vec[j] + ((arr[i] - vec[0] + Md) % Md)) % Md]++;
		ok = 1;
		for (auto it :check){
			if (it.second != frq[it.first]) {
				ok = false;
				break;
			}
		}
