...
void Read() {
	...
	lop(i, 0, n) sc(a[i]), b[i] = a[i];
	lop(i, 1, n) a[i] += a[i - 1];
}
void Process() {
	mx = 0, ans = 1;
	lop(j, 0, n) {
		ll hem = distance(a.begin(), upper_bound(a.begin(), a.end(), s));
		if (hem >= j) hem = distance(a.begin(), upper_bound(a.begin(), a.end(), s + b[j])) - 1;
		if (hem > mx) mx = hem, ans = j + 1;
	}
}
void GetAC() {
	Read();
	Process();
	if (distance(a.begin(), upper_bound(a.begin(), a.end(), s)) > mx) mx = distance(a.begin(), upper_bound(a.begin(), a.end(), s)), ans = 0;
	print(ans);
}
