...
string FromDecimalToAnyBase(ll n, int base) { if (n == 0) re "0";  string ret = ""; while (n) { ret = Letters[n%base] + ret;  n /= base; } re ret; }
...
ll DIVMOD(ll x, ll y) { return (x%MOD)*PowMod(y, MOD - 2, MOD) % MOD;}
...
void Process() {
	lop(i, 0, n) lop(ii, 0, a[i]) {
		ll ok = hem[arr[i][ii]], ok2 = n * a[i];
		ret += DIVMOD(ok, ok2);
		ret %= MOD;
	}
}
