...
bool isPalindexrome(string str) { ll l = 0; ll h = str.sz - 1; while (h > l) { if (str[l++] != str[h--]) { re 0; } } re 1; }
...
void GO() {
	...
	Hem.PB(xi);
	while (k > 0) {
		if (k > index*(Hem[index - 1])) {
			v[mid] = v[mid + index];
			k = k - index*Hem[index - 1];
		}
		else {
			v[mid] += k / index;
			k = 0;
