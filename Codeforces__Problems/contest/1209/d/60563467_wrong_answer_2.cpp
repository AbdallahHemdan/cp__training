...
int n, m, a, b;
pii All[maxn];
int frq[maxn], cntW[maxn], vis[maxn];
int getcnt = 0;
...
int main()
{
	READ();
	lop(i,1,m) {
		int cur_1 = All[i].first;
		int cur_2 = All[i].y;
		if (!cntW[cur_1]) {
			arr.PB(MP(frq[cur_1], cur_1));
			cntW[cur_1] = 0;
		}
		if (cntW[cur_2] == 0) {
			arr.PB(MP(frq[cur_2], cur_2));
			cntW[cur_2] = 0;
		}
	}
	sort(all(arr));
	lop(i,0,arr.sz){
		if (!vis[arr[i].y]) DFS(arr[i].y);
	}
	print(getcnt - m);
}