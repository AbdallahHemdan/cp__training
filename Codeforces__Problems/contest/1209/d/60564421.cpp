...
* I hope my code compiles and GODFSs accepted.
*  ___   __
* |\  \|\  \
* \ \  \_\  \
*  \ \   ___ \emdan
...
int main() {
	...
	READ();
	lop(i,1,m+1) {
		int cur_1 = mnn[i].first;
		int cur_2 = mnn[i].second;
		if (od[cur_1] == 0)
		{
			hem.pb(mp(frq[cur_1], cur_1));
			od[cur_1] = 0;
		}
		if (od[cur_2] == 0) {
			hem.pb(mp(frq[cur_2], cur_2));
			od[cur_2] = 0;
		}
	}
	sort(all(hem));
	lop(i,0,hem.sz) {
		if (vis[hem[i].second] == 0) {
			DFS(hem[i].second);
		}
	}
	cout << wyn - m << endl;
