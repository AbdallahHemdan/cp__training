...
ll PowMod(ll j, ll ex, ll M) { if (!ex)return 1; ll a = PowMod((j*j) % M, ex >> 1, M); re(ex & 1) ? (a*j) % M : a; }
...
void get() {
	ret = 0;
	for (auto it : ok) if (it.second > 1) ret = true;
	(ret) ? print("NO") : print("YES");
}
void getAC_ISA() {
	read();
	go();
