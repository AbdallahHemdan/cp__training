...
const ll N = 1e3 + 9; 
ll dotCount[N];
ll dp[N][N][3];  // 0- dot  // 1- hash // 2-nothing
ll GetAns(ll col, ll wid, ll last) {
	if (col == m) {
		if (wid > y || wid < x) re OO; 
		else re  0; 
	}
	ll& ret = dp[col][wid][last];
	if (~ret) re ret;  
	ll dot = OO;
	if (last == 2 || (last == 0 && wid + 1 <= y) || (last == 1 && wid >= x))
		dot = n - dotCount[col] + GetAns(col + 1, (last == 0) ? wid + 1 : 1, 0);

	ll hash = OO;
	if (last == 2 || (last == 1 && wid + 1 <= y) || (last == 0 && wid >= x))
		hash = dotCount[col] + GetAns(col + 1, (last == 1) ? wid + 1 : 1, 1);
