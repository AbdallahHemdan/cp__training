...
#define in insert
#define sc(n) cin >> n
#define sc2(n,MD) cin >> n >> MD
#define sc3(a,b,c) cin >> a >> b >> c
#define sc4(a,b,c,d) cin >> a >> b >> c >> d
#define watch(x) cout << (#x) << " is " << (x) << endl
#define stringToLower(a)  transform(all(a), a.begin(), ::tolower);
#define stringToUpper(a)  transform(all(a), a.begin(), ::toupper);
...
ll FromAnyBaseToDecimal(string inp, int base) { ll ret = 0;  lop(ii, 0, inp.sz) { ret *= base; ret += toInt(inp[ii]); } return ret; }
...
ll PowMod(ll bs, ll ex, ll M) { if (!ex)return 1; ll lg = PowMod((bs*bs) % M, ex >> 1, M); re(ex & 1) ? (lg*bs) % M : lg; }
...
int go2(ll ii) {
	ll aa = n - b[ii]; aa++;
	if (pos > aa)  re print("-1"), 0;
	ret.PB(pos);
	pos += a[ii];
	return 1;
}
void getRet() {
	lop(ii, 0, m) if (!go2(ii)) return;
	prt();
}
void getAC_ISA() {	
	read();
	if (acu < n)  re print("-1"), void();
	get(); getRet();
}
