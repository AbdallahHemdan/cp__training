...
void setIt(ll i) {
	frq[a[i]] = 1, mx = max(a[i], mx);
	if (mx == i) sv[i][0] = 1;
}
void go1() {
	lop(i, 1, n + 1) {
		if (frq.count(a[i]) == 0) setIt(i);
		else break;
	}
	frq.clear();
	mx = 0;
...
void getRet() {
	lop(i, 2, n + 1) {
		if (sv[i - 1][0]) if (sv[i][1]) print2(i - 1, n - i + 1);
		sv[i - 1][0] = sv[i - 1][1] = sv[i][1] = 0;
	}
	sv[n][0] = 0;
}
