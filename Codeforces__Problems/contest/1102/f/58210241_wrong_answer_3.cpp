...
int DP(int mask, int v)
{
	...
	f[mask][v] = 0;
	for (int i = 0; i<n; i++) {
		if (i != v && (mask >> i & 1)) f[mask][v] = max(f[mask][v], min(c1[i + 1][v + 1], DP(mask ^ (1 << v), i)));
	}
	return f[mask][v];
}
int main()
{
	...
	lop(i, 1, n + 1) {
		lop(j, 1, n + 1) {
			...
			int t = 1e9 + 7;
			lop(k, 1, m + 1)
				t = min(t, abs(a[i][k] - a[j][k]));
			c1[i][j] = t;
			t = 1e9 + 7;
			lop(k, 1, m) {
				t = min(t, abs(a[i][k] - a[j][k + 1]));
			}
			c2[i][j] = t;
		}
	}
	...
	lop(i,1,n+1){
		mms(f, -1);
		lop(j, 1, n + 1)
			f[1 << (j - 1)][j - 1] = (i == j ? 1e9 + 7 : 0);
		lop(j, 1, n + 1)
			ans = max(ans, min(c2[j][i], DP((1 << n) - 1, j - 1)));
	}
	cout << ans;
}