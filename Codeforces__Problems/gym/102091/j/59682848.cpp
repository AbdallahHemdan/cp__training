...
/**************************/
/*Dear online judge:
* I've read the problem, and tried to GetAns it.
...
#define mms(Arr, hemue) memset(Arr, hemue, sizeof(Arr))
...
ll loc;
void PrintAns() {
	lop(i, 0, arr.sz) cout << arr[i];
	print("");
}
void GetAns() {
	vec.clear(), arr.clear();
    while(n > 0) {
		loc = distance(hem, upper_bound(hem, hem + maxn + 1, n));
		n = n - hem[loc - 1];
		vec.PB(loc - 1);
    }
    ll tmp(0);
