...
int main() {
	...
	lop(i, 0, p) cin >> a[i].first >> a[i].second;
	lop(i, 0, q) cin >> b[i].first >> b[i].second; 

	lop(i, 0, p) {
		lop(j, a[i].first, a[i].second + 1) vis[j] = true;
	}
	...
	ll cnt = 0; 
 	lop(k, l, r + 1) {
		bool flag = false;
		lop(i, 0, q) {
			lop(j, b[i].first + k, b[i].second + 1 + k)
				if (vis[j]) {
					flag = true; 
					break;
				}
